...
os.makedirs('/net/server/data/Archive/prob_learn/asmyasnikova83/hp_early_trials_prev/{0}/{0}_ave_into_subj'.format(freq_range), exist_ok = True)
...
for subj in subjects:
    for t in trial_type:
        ...
        for r in rounds:
            try:
                epochs_positive = mne.read_epochs('/net/server/data/Archive/prob_learn/asmyasnikova83/hp_early_trials_prev/{0}/{0}_epo/{1}_run{2}_{3}_fb_cur_positive_{0}_epo.fif'.format(freq_range, subj, r, t), preload = True)             
        ...
        for r in rounds:
            try:
                ...
                epochs_negative = mne.read_epochs('/net/server/data/Archive/prob_learn/asmyasnikova83/hp_early_trials_prev/{0}/{0}_epo/{1}_run{2}_{3}_fb_cur_negative_{0}_epo.fif'.format(freq_range, subj, r, t), preload = True)
        ...
        if data_into_subj.size != 0:
            ...
            temp.save('/net/server/data/Archive/prob_learn/asmyasnikova83/hp_early_trials_prev/{0}/{0}_ave_into_subj/{1}_{2}_evoked_{0}_resp.fif'.format(freq_range, subj, t))
