...
def call_clean_events(data_path, subj, r, cond, fb):
    ...
    events = np.loadtxt('/net/server/data/Archive/prob_learn/asmyasnikova83/mio_cond_prev_fb_pos/Events/{0}_run{1}_{2}_fb_{3}.txt'.format(subj, r, cond, fb), dtype='int')
    # (3,) -> (1,3)
    if events.shape == (3,):
    ...
    if clean.any():
        ...
        mio_corrected_events_file = open('/net/server/data/Archive/prob_learn/asmyasnikova83/mio_cond_prev_fb_pos/Events_mio/{0}_run{1}_{2}_fb_{3}.txt'.format(subj, r, cond, fb), "w")
...
os.makedirs('/net/server/data/Archive/prob_learn/asmyasnikova83/mio_cond_prev_fb_pos/Events_mio/', exist_ok = True)

for subj in subjects:
    for r in rounds:
        for cond in trial_type:
            for fb in feedback:
                rf = '/net/server/data/Archive/prob_learn/asmyasnikova83/mio_cond_prev_fb_pos/Events/{0}_run{1}_{2}_fb_{3}.txt'.format(subj, r, cond, fb) 
