...
os.makedirs('/net/server/data/Archive/prob_learn/asmyasnikova83/hp_early_trials/{0}/{0}_ave_into_subj_comb_planar/'.format(freq_range), exist_ok = True)
for t in trial_type:
    for fb in feedback:
        for subj in subjects:
            try:
                evoked = mne.Evoked('/net/server/data/Archive/prob_learn/asmyasnikova83/hp_early_trials/{0}/{0}_ave_into_subj/{1}_{2}_{0}_resp_fb_cur_{3}.fif'.format(freq_range, subj, t, fb))
                ...
                donor.save('/net/server/data/Archive/prob_learn/asmyasnikova83/hp_early_trials/{0}/{0}_ave_into_subj_comb_planar/{1}_{2}_evoked_{0}_resp_comb_planar_fb_cur_{3}.fif'.format(freq_range, subj, t, fb))
