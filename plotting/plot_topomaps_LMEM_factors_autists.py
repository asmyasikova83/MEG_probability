...
for i in range(102):
    ...
    pval_group = pval_s['group'].tolist()
    pval_in_intevals.append(pval_group)
...
title = 'group, LMEM %s Hz, noFDR'%freq_range
...
title = 'group, LMEM %s Hz,  space FDR'%freq_range
...
title = 'group, LMEM %s Hz, full FDR'%freq_range
...
fig.savefig(f'{path_pic}/LMEM_group_stat_{freq_range}_no_fdr.jpeg', dpi = 900)
fig2.savefig(f'{path_pic}/LMEM_group_stat_{freq_range}_space_fdr.jpeg', dpi = 900)
fig3.savefig(f'{path_pic}/LMEM_group_stat_{freq_range}_full_fdr.jpeg', dpi = 900)
############### 1. feedback_cur:group #####################################
################# p_value ########################
pval_in_intevals = []
# number of heads in line and the number og intervals into which we divided (see amount od tables with p_value in intervals)
for i in range(102):
    pval_s = df[df['sensor'] == i]
    pval_feedback_cur_group = pval_s['feedback_cur:group'].tolist()
    pval_in_intevals.append(pval_feedback_cur_group)
...
#no fdr
binary = p_val_binary(pval_in_intevals, treshold = 0.05)
title = 'feedback_cur:group, LMEM %s Hz, noFDR'%freq_range
fig = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

# space fdr
binary_space = p_val_binary(pval_space_fdr, treshold = 0.05)
title = 'feedback_cur:group, LMEM %s Hz,  space FDR'%freq_range
fig2 = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary_space), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

# full fdr
binary_full = p_val_binary(pval_full_fdr, treshold = 0.05)
title = 'feedback_cur:group, LMEM %s Hz, full FDR'%freq_range
fig3 = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary_full), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

fig.savefig(f'{path_pic}/LMEM_feedback_cur_stat_{freq_range}_no_fdr.jpeg', dpi = 900)
fig2.savefig(f'{path_pic}/LMEM_feedback_cur_stat_{freq_range}_space_fdr.jpeg', dpi = 900)
...
for i in range(102):
    pval_s = df[df['sensor'] == i]
    pval_trial_type_group = pval_s['trial_type:group'].tolist()
    pval_in_intevals.append(pval_trial_type_group)
...
#no fdr
binary = p_val_binary(pval_in_intevals, treshold = 0.05)
title = 'trial_type:group, LMEM %s Hz, noFDR'%freq_range
fig = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

# space fdr
binary_space = p_val_binary(pval_space_fdr, treshold = 0.05)
title = 'trial_type:group, LMEM %s Hz,  space FDR'%freq_range
fig2 = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary_space), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

# full fdr
binary_full = p_val_binary(pval_full_fdr, treshold = 0.05)
title = 'trial_type:group, LMEM %s Hz, full FDR'%freq_range
fig3 = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary_full), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

fig.savefig(f'{path_pic}/LMEM_trial_type_stat_{freq_range}_no_fdr.jpeg', dpi = 900)
fig2.savefig(f'{path_pic}/LMEM_trial_type_stat_{freq_range}_space_fdr.jpeg', dpi = 900)
fig3.savefig(f'{path_pic}/LMEM_trial_type_stat_{freq_range}_full_fdr.jpeg', dpi = 900)


############### 3. trial_type:feedback_cur:group #####################################
################# p_value ########################
pval_in_intevals = []
# number of heads in line and the number og intervals into which we divided (see amount od tables with p_value in intervals)
for i in range(102):
    ...
    pval_s = df[df['sensor'] == i]
    pval_trial_type_feedback_cur_group = pval_s['trial_type:feedback_cur:group'].tolist()
    pval_in_intevals.append(pval_trial_type_feedback_cur_group)
...
#no fdr
binary = p_val_binary(pval_in_intevals, treshold = 0.05)
title = 'trial_type:feedback_cur:group, LMEM %s Hz, noFDR'%freq_range
fig = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

# space fdr
binary_space = p_val_binary(pval_space_fdr, treshold = 0.05)
title = 'trial_type:feedback_cur:group, LMEM %s Hz, space FDR'%freq_range
fig2 = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary_space), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))

# full fdr
binary_full = p_val_binary(pval_full_fdr, treshold = 0.05)
title = 'trial_type:feedback_cur:group, LMEM %s Hz, full FDR'%freq_range
fig3 = temp.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.9, vmax = 0.9, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary_full), mask_params = dict(marker='o', markerfacecolor='green', markeredgecolor='k', linewidth=0, markersize=10, markeredgewidth=2))
