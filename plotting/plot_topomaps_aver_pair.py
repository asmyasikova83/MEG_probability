...
for p in planars:
    print(data_path)
    t_stat, p_val, risk_mean, norisk_mean = ttest_pair(data_path, subjects, fr, parameter1 = f'{cond1}', parameter2 = f'{cond2}', parameter3 = parameter3, parameter4 = parameter4, planar = p,  n = n)
    if parameter3 == 'negative':
       title = (f'In {cond1} feedback negative vs positive, noFDR')
    ...
    pval_in_intevals = pval_in_intevals
    ...
    #data_to_sum = data_for_plotting[:]/2
    #print(data_to_sum[:, np.newaxis])
    ...
    #plotting_LMEM = mne.EvokedArray(data_to_sum, info = temp.info)
    plotting_LMEM.times = times

    binary = p_val_binary(pval_in_intevals, treshold = 0.05)
    if parameter3 == 'negative':
        title = (f'In {parameter1} fb neg vs pos')
    if parameter3 == None:
        #title = (f'{cond1} vs {cond2}, %s, LMEM, noFDR'%p)
        print('Making pic')
        title = (f'{cond1} vs {cond2}')
    fig = plotting_LMEM.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'dB', show = False, vmin = -0.5, vmax = 0.5, time_unit='ms', title = title, colorbar = True, extrapolate = "local", mask_params = dict(marker='o',		markerfacecolor='white', markeredgecolor='k', linewidth=0, markersize=7, markeredgewidth=2))
    print('Pic ready')
    # space fdr
    ...
    if parameter3 == 'negative':
        ...
        fig.savefig(f'/net/server/data/Archive/prob_learn/asmyasnikova83/beta/topomaps_lines_ttest_sum/{cond1}/LMEM_{cond1}_vs_{parameter3}_vs_{parameter4}_stat_no_fdr_{fr}_separ_fb.jpeg', dpi = 900)
        #fig3.savefig(f'/net/server/data/Archive/prob_learn/asmyasnikova83/beta/topomaps_lines_ttest_sum/{cond1}/LMEM_{cond1}_vs_{parameter3}_vs_{parameter4}_stat_full_fdr_{fr}_separ_fb.jpeg', dpi = 900)
    if parameter3 == None:
        os.makedirs(f'/net/server/data/Archive/prob_learn/asmyasnikova83/beta/topomaps_lines_ttest_sum_Jan_2022/{cond1}_vs_{cond2}/' , exist_ok = True)
        fig.savefig(f'/net/server/data/Archive/prob_learn/asmyasnikova83/beta/topomaps_lines_ttest_sum_Jan_2022/{cond1}_vs_{cond2}/LMEM_{cond1}_vs_{cond2}_stat_no_fdr_{fr}_separ_fb.jpeg', dpi = 900)
        #fig2.savefig(f'/net/server/data/Archive/prob_learn/asmyasnikova83/beta/topomaps_lines_ttest_sum/decision_making_{cond1}_vs_{cond2}/LMEM_{cond1}_vs_{cond2}_stat_space_fdr_{fr}_separ_fb.jpeg', dpi = 900)
        #fig3.savefig('/net/server/data/Archive/prob_learn/asmyasnikova83/{0}/topomaps_lines_LMEM_{1}_poster/norisk_vs_risk/LMEM_norisk_vs_risk_stat_full_fdr_{2}_separ_fb.jpeg'.format(feedb, fr, p), dpi = 300)
        #fig3.savefig(f'/net/server/data/Archive/prob_learn/asmyasnikova83/beta/topomaps_lines_ttest_sum/{cond1}_vs_{cond2}/LMEM_{cond1}_vs_{cond2}_stat_full_fdr_{fr}_separ_fb.jpeg', dpi = 900)
