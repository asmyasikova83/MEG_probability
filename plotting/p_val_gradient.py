...
for p in planars:
    ...
    if parameter3 == None:
       if response:
           df = pd.read_csv(f'/{prefix}/beta/p_vals_Tukey_condition_contrast_MEG.csv')
       else:
    ...
    fig3 = plotting_LMEM.plot_topomap(times = time_to_plot, ch_type='planar1', scalings = 1, units = 'p-val', show = False, vmin = 0.45, vmax = 1.0, time_unit='s', title = title, colorbar = True, extrapolate = "local", mask = np.bool_(binary_full), mask_params = dict(marker='o', markerfacecolor='yellow', markeredgecolor='w', linewidth=0, markersize=5, markeredgewidth=2))

    if parameter3 == 'negative':
        os.makedirs(f'/{prefix}/topomaps/topomaps_rows_LMEM_response/p_val/{cond1}/', exist_ok = True)
        fig3.savefig(f'/{prefix}/topomaps/topomaps_rows_LMEM_response/p_val/{cond1}/LMEM_{cond1}_vs_{parameter3}_vs_{parameter4}_stat_full_fdr_{fr}.jpeg', dpi = 300)
    if parameter3 == None:
        if response:
            os.makedirs(f'/{prefix}/topomaps/topomaps_rows_LMEM_response/p_val/{cond1}_vs_{cond2}/' , exist_ok = True)
            fig3.savefig(f'/{prefix}/topomaps/topomaps_rows_LMEM_response/p_val/{cond1}_vs_{cond2}/LMEM_{cond1}_vs_{cond2}_stat_full_fdr_{fr}.jpeg', dpi = 300)
