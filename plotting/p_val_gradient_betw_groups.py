...
from function import ttest_pair, ttest_pair_independent, space_fdr, full_fdr, p_val_binary, plot_deff_topo, plot_topo_vs_zero
from config import *
...
path_pic = '/net/server/data/Archive/prob_learn/asmyasnikova83/topomaps_LMEM_betw_groups_600ms_pval_grad/{0}'.format(fr)
...
for p in planars:
    ...
    _, _, comp1_mean, comp2_mean, comp1, comp2 = ttest_pair_independent(data_path_AT, data_path_NT, response, Autists, Normals, fr, fr, parameter1 = f'{cond1}', parameter3=parameter3, planar=p, n=n)
    ...
    if parameter3 == None:
        ...
        df = pd.read_csv(f'/{prefix_data}/beta/p_vals_by_trial_type_betw_group_MEG_600ms_fin.csv')
...
temp.data = comp1_mean - comp2_mean
...
for i in range(102):
    ...
    if parameter3 == None:
        pval_norisk_risk = pval_s[f'{cond1}-normals_autists'].tolist()
...
if nofdr:
    ...
    if parameter3 == 'negative':
        title = (f'{cond1} AT minus NT, %s, {line}, noFDR'%p)
    else:
        title = (f'{cond1} AT minus NT, %s, {line}, noFDR'%p)
if spacefdr:
    types = 'space'
    ...
    if parameter3 == 'negative':
        title = (f'{cond1} AT minus NT, %s, {line}, spaceFDR'%p)
    else:
        title = (f'{cond1} AT minus NT, %s, {line}, spaceFDR'%p)
if fullfdr:
    types = 'full'
    ...
    if parameter3 == 'negative':
        title = (f'{cond1} AT minus NT, %s, {line}, fullFDR'%p)
    else:
        title = (f'{cond1} AT minus NT, %s, {line}, fullFDR'%p)
...
if parameter3 == 'negative':
     ...
     fig3.savefig(f'/{path_pic}/{cond1}/{stat}_{cond1}_vs_{parameter3}_vs_{parameter4}_stat_{types}_fdr_{fr}.jpeg', dpi = 300)
if parameter3 == None:
    if response:
        os.makedirs(f'/{path_pic}/{cond1}_vs_{cond2}/' , exist_ok = True)
        fig3.savefig(f'/{path_pic}/{cond1}_vs_{cond2}/{stat}_{cond1}_vs_{cond2}_stat_{types}_fdr_{fr}.jpeg', dpi = 300)
        print('Saved!')
