...
df = pd.read_csv('/net/server/data/Archive/prob_learn/asmyasnikova83/gamma_30_50/stc/lmem_label/tukey_label_1500_1900_betweengroup.csv')

src = mne.setup_source_space(subject = "fsaverage", spacing="ico5", add_dist=False)

###### create plot  with p_value with marginal means ######
data =df['positive-norisk-autists - normal']
data = data.drop([448])
data = np.array(data)
data[data>0.05]=1
...
fdr_stc = mne.labels_to_stc(labels,space_arr[:,1], tmin = 1.500, tstep = 0.1)
...
brain.save_image('/net/server/data/Archive/prob_learn/asmyasnikova83/gamma_30_50/stc/lmem_label/hp_pos.jpeg')
brain.close()
