...
def make_subjects_df(prefix, combined_planar, s, subj, r, t, fb_cur, tmin, tmax, step, scheme):
    ...
    feedback_prev_data = np.loadtxt("/{0}/prev_fb_mio_corrected/{1}_run{2}_{3}_fb_cur_{4}_prev_fb.txt".format(prefix, subj, r, t, fb_cur), dtype='int')
    ...
    a = scheme.loc[(scheme['subject'] == subj) & (scheme['block'] == r)]['scheme'].tolist()[0]
...
#############################################################################
##################### непарный ttest #######################################	
def ttest_vs_zero_test(data_path, subjects, parameter1, freq_range, planar, n): # n - количество временных отчетов
