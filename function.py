...
def make_beta_signal_changed_bline(subj, r, cond, fb, data_path, L_freq, H_freq, f_step, period_start, period_end, baseline, n_cycles, time_bandwidth = 4):
    freqs = np.arange(L_freq, H_freq, f_step)
...
def make_beta_signal(prefix_data, prefix, subj, r, cond, fb, data_path, L_freq, H_freq, f_step, period_start, period_end, baseline, n_cycles, time_bandwidth = 4):
    freqs = np.arange(L_freq, H_freq, f_step)
    print("/{0}/fix_cross_mio_corr{1}/{2}_run{3}_norisk_fb_cur_positive_fix_cross.txt".format(prefix_data, prefix, subj, r))
    events_pos = np.loadtxt("/{0}/fix_cross_mio_corr{1}/{2}_run{3}_norisk_fb_cur_positive_fix_cross.txt".format(prefix_data, prefix, subj, r), dtype='int') 
    print('eve pos', events_pos) 
    # если только одна метка, т.е. одна эпоха, то выдается ошибка, поэтому приводим shape к виду (N,3)
    if events_pos.shape == (3,):
        events_pos = events_pos.reshape(1,3)
    # download marks of negative feedback 
    events_neg = np.loadtxt("/{0}/fix_cross_mio_corr{1}/{2}_run{3}_norisk_fb_cur_negative_fix_cross.txt".format(prefix_data, prefix, subj, r), dtype='int')
    print('eve neg', events_neg)
    # если только одна метка, т.е. одна эпоха, то выдается ошибка, поэтому приводим shape к виду (N,3)
    if events_neg.shape == (3,):
        events_neg = events_neg.reshape(1,3)
        #объединяем негативные и позитивные фидбеки для получения общего бейзлайна по ним, и сортируем массив, чтобы времена меток шли в порядке возрастания 
    ...
    events = np.unique(events, axis = 0)
    #events, which we need
    print('/{0}/Events_mio_list_compare{1}/{2}_run{3}_{4}_fb_cur_{5}.txt'.format(prefix_data, prefix, subj, r, cond, fb))
    events_response = np.loadtxt('/{0}/Events_mio_list_compare{1}/{2}_run{3}_{4}_fb_{5}.txt'.format(prefix_data, prefix, subj, r, cond, fb), dtype='int')
    print('events resp', events_response)
    # если только одна метка, т.е. одна эпоха, то выдается ошибка, поэтому приводи shape к виду (N,3)
    if events_response.shape == (3,):
        events_response = events_response.reshape(1,3)
    #remove doubles with the same timing
