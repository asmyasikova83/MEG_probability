...
os.makedirs('{0}/stim_check/{1}_feedback/{1}_ave_comb_planar'.format(prefix, freq_range), exist_ok = True)

for t in trial_type:
    for fb in feedback:
        for subj in subjects:
            try:
                evoked = mne.Evoked('{0}/stim_check/{1}_feedback/{1}_ave_into_subj/{2}_{3}_{1}_resp_fb_cur_{4}.fif'.format(prefix, freq_range, subj, t, fb))
                _, _, comb_planar = combine_planar_Evoked(evoked)
                donor.data = comb_planar
                donor.save('{0}/stim_check/{1}_feedback/{1}_ave_comb_planar/{2}_{3}_fb_cur_{4}_evoked_beta_16_30_resp_comb_planar.fif'.format(prefix, freq_range,subj, t, fb))
            except (OSError):
