...
os.makedirs('{0}/stim_check/{1}/{1}_second_bl_epo'.format(prefix_sasha, freq_range), exist_ok = True)
...
with open('{0}/stim_check/{1}/{1}_second_bl_epo/config.txt'.format(prefix_sasha, freq_range), "w") as file:
    for  line in lines:
        file.write(line + '\n')
    ...
    for r in rounds:
        for cond in trial_type:
            ...
            try:
                ...
                print('{0}/stim_check/{1}/{1}_epo/{2}_run{3}_{4}_{1}_epo.fif'.format(prefix_sasha, freq_range, subj, r, cond))
                epochs_tfr = mne.read_epochs('{0}/stim_check/{1}/{1}_epo/{2}_run{3}_{4}_{1}_epo.fif'.format(prefix_sasha, freq_range, subj, r, cond), preload = True)    
	        ...
	        # Для бейзлайна меняем оси местами, на первом месте число каналов
                ...
                #epochs_second_bl.save('/net/server/data/Archive/prob_learn/asmyasnikova83/stim_check/{0}/{0}_second_bl_epo/{1}_run{2}_{3}_fb_cur_{4}_{0}_epo.fif'.format(freq_range, subj, r, cond, fb), overwrite=True)
                epochs_second_bl.save('{0}/stim_check/{1}/{1}_second_bl_epo/{2}_run{3}_{4}_{1}_epo.fif'.format(prefix_sasha, freq_range, subj, r, cond), overwrite=True)
            except (OSError):
