...
# choose sensor
# whole 15, anter 5, occip 15, posterror 16
sensor_choosen = 15
...
m <- lmer(get(interval) ~ trial_type*feedback_prev + (1|subject),data = means) # main part, fit model!
m2 <- m # it is because we missed step, so final model=initial model
print(anova(m2))
Tuk1 <- emmeans(m2, pairwise ~ feedback_prev | trial_type, adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000)
...
y_values_rew <- means[feedback_prev == 'positive', mean(get(interval)),by='trial_type']
y_values_lose <- means[feedback_prev == 'negative', mean(get(interval)),by='trial_type']
...
y_values_rew <- means[feedback_prev == 'positive',
                      mean(get(interval)),by='trial_type']
setnames(y_values_rew,'V1','y_values_rew')

y_values_lose <- means[feedback_prev == 'negative',
                       mean(get(interval)),by='trial_type']
setnames(y_values_lose,'V1','y_values_lose')
...
p1 <- ggline(means, 'trial_type', interval,
  ...
  geom_hline(yintercept=-0.0, linetype='dashed', col = 'black', size = 2)+
...
ggsave(filename = paste0(out_path,'_Tukey_feedback_FIG6_anter_prev','.png'), width =  6, height = 5)
