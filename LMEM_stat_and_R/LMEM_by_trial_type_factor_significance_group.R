...
#for (i in 1:2){
  ...
  for (j in cols){
    m <- lmer(get(j) ~ trial_type*group + (1|subject), data = temp_bind)
    ...
    Tuk <- data.table(summary(emmeans(m, pairwise ~ group | trial_type, adjust = 'tukey',lmer.df = "satterthwaite",lmerTest.limit=8000))$contrasts)
...
write.csv(p_vals,paste0(out_path, "p_vals_by_trial_type_MEG_group_600ms_fin.csv"))