...
def fixation_cross_events(data_path_raw, raw_name, data_path_events, name_events, subj, r, t, fb):
    ...
    print(op.join(data_path_events, name_events.format(subj, r, t, fb)))
    no_risk = np.loadtxt(op.join(data_path_events, name_events.format(subj, r, t, fb)), dtype='int')
    #no_risk = read_events(op.join(data_path_events, name_events.format(subj, r)))
    
    # Load raw events without miocorrection for p001-p062
    #events_raw = read_events(op.join(data_path_raw, raw_name.format(subj, r)))
    # Load raw events without miocorrection for p063-p067
