...
# for p_value
lines = ["resample: {}".format(resample), "time_points: {}".format(time_points), "scale_pvalue: {}".format(scale_pvalue), "scale_mean_beta_between_fb : {}".format(scale_mean_beta_between_fb), "scale_mean_beta_between_choice_types : {}".format(scale_mean_beta_between_choice_types), "method: sLoreta"]


with open("/net/server/data/Archive/prob_learn/asmyasnikova83/Sources/beta_16_30/plot_sources_ttest/config.txt", "w") as file:
    for  line in lines:
        file.write(line + '\n')
...
for cond in trial_type:
    for ind, v in enumerate(var_of_ploting):
        stc_pos_vs_neg = mne.read_source_estimate('/net/server/data/Archive/prob_learn/asmyasnikova83/Sources/beta_16_30/ttest_pos_vs_neg/{0}_fb_cur_pos_vs_neg_{1}'.format(cond, v), 'fsaverage')
        ...
        for t in time_points:
            ...
            brain.save_image('/net/server/data/Archive/prob_learn/asmyasnikova83/Sources/beta_16_30/plot_sources_ttest/{0}_{1}_neg_vs_pos_{2}.jpeg'.format(t, cond, v))
            brain.close()
